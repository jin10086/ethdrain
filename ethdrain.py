...
if __name__ == "__main__":
    ...
    def http_post_request(url, request):
        return requests.post(
            url, data=request, headers={"content-type": "application/json"}
        ).json()
    ...
    parser.add_argument(
        "-r", "--ethrpcurl", default=ETH_URL, help="The Ethereum RPC node url and port."
    )
    ...
    if not args.end_block:
        args.end_block = (
            int(
                http_post_request(ETH_URL, Ethdrain.make_request("latest", False))[
                    "result"
                ]["number"],
                0,
            )
    ...
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    print(
        "Processing {} blocks split into {} chunks on {} processes:".format(
